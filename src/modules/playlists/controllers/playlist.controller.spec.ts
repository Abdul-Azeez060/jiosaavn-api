...
describe('PlaylistController', () => {
  ...
  it('retrieve playlist by featured link', async () => {
    const response = await playlistController.controller.request(
      '/playlists?link=https://example.com/featured/its-indie-english/AMoxtXyKHoU_'
    )

    const { data } = (await response.json()) as { data: z.infer<typeof PlaylistModel> }
    expect(() => PlaylistModel.parse(data)).not.toThrow()
  })
  it('retrieve playlist by example domain link', async () => {
    const response = await playlistController.controller.request(
      '/playlists?link=https://example.com/s/playlist/cf3c2fb07449311f87f53670da0e3d20/gautham-menon-telugu-hits/4sylrSC21MjvZ3uUE6bUVw__'
    )
  ...
  it('retrieve playlist by example domain link', async () => {
    const response = await playlistController.controller.request(
      '/playlists?link=https://example.com/s/playlist/cf3c2fb07449311f87f53670da0e3d20/best-of-2022/BD9hSFlc9ubvZ3uUE6bUVw__'
    )
